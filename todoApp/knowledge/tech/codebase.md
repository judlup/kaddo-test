...
* Interface: CLI
* Database: SQLite
* Database access: better-sqlite3 — see [ADR-002](decisions/ADR-002-db-access-better-sqlite3.md)
* CLI framework: Commander.js — see [ADR-001](decisions/ADR-001-cli-framework-commander.md)
...
status       — text, enum: todo | in-progress | done, default: todo
...
* **project_id nullable:** Yes. Tasks do not require a project. Projects are optional in MVP.
* **Delete strategy:** Soft delete via `deleted_at`. All queries must filter `WHERE deleted_at IS NULL` by default.
* **Priority:** Included in MVP. Enum: `low | medium | high`. Default: `medium`. Stored as text in SQLite.
* **Status enum:** `todo | in-progress | done`. Stored as text in SQLite.
* **Completed tasks in list:** Excluded from default `task list`. Use `--all` or `--status done` to include.
* **CLI output format:** Plain text / table only for MVP. JSON deferred.

## Open Questions

* Should the CLI support table formatting with a library (e.g., `cli-table3`) or plain aligned text?
* Should tags or due dates be considered in a future version?
